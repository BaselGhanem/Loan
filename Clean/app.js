(() => {
  ...
  function calculateDerived(currentState) {
    ...
    const whatIf = simulateLoan(currentState, {
      ...
      oneTimePayment: oneTimeAmount > 0 ? { date: oneTimeDate, amount: oneTimeAmount, method: "reduceTerm", note: "دفعة تجربة" } : null,
  ...
  function simulateLoan(currentState, options = {}) {
    ...
    for (let monthIndex = 1; monthIndex <= MAX_MONTHS; monthIndex++) {
      ...
      if (monthIndex <= gracePeriod) {
        notes.push("فترة سماح: تراكم فائدة بدون قسط");
      } else if (dueThisMonth) {
        scheduledPayment = Math.min(balance, currentPeriodPayment + Math.max(0, num(options.extraMonthly)));
        principalPaid = Math.max(0, scheduledPayment - interest);
  ...
  function renderDashboard() {
    ...
    const budgetImpact = monthlyBudget > 0 ? safePercent(current.paymentAmount, monthlyBudget) : 0;

    const cards = [
      { label: "القسط الحالي", value: money(current.paymentAmount), hint: state.loan.paymentFrequency === "quarterly" ? "قسط ربع سنوي" : "قسط شهري" },
      { label: "إجمالي الفائدة", value: money(current.totalInterest), hint: `الخطة قبل: ${money(original.totalInterest)}` },
      ...
      { label: monthsSaved >= 0 ? "الأشهر الموفرة" : "أشهر إضافية", value: `${Math.abs(Math.round(monthsSaved))} شهر`, hint: "مقارنة بالخطة قبل" },
      ...
      { label: "الفائدة الآن", value: `${formatNumber(currentRate, 2)}%`, hint: state.loan.interestType === "variable" ? "قد تتغير حسب الفترة" : "ثابتة" },
    ...
    const insights = [
      {
        ...
        body: current.remainingBalance <= EPS
          ? "القرض مكتمل حسابياً حسب البيانات الآن. احتفظ بنسخة JSON قبل أي تعديل كبير."
          : `الرصيد الحالي المتوقع ${money(current.remainingBalance)}، وتاريخ الإغلاق ${dateLabel(current.payoffDate)}.`
      },
      {
        title: "أثر الخطة الآن",
        body: interestSaved >= 0
          ? `الوضع الحالي يوفر تقريباً ${money(interestSaved)} و ${Math.max(0, Math.round(monthsSaved))} شهر مقارنة بالخطة قبل.`
          : `الوضع الحالي يزيد التكلفة بحوالي ${money(Math.abs(interestSaved))}. راقب فترات الفائدة أو جرّب إعادة التمويل.`
      },
      {
        title: "ماذا أفعل؟",
        body: bestStrategy
  ...
  function renderRates() {
    ...
    els.rateImpact.innerHTML = comparisonCards([
      { label: "الخطة قبل", value: money(base.totalInterest), hint: `${base.monthsElapsed} شهر` },
  ...
  function renderPayments() {
    ...
    els.paymentImpact.innerHTML = comparisonCards([
      ...
      { label: "الأشهر الموفرة", value: `${Math.max(0, Math.round(derived.monthsSaved))} شهر`, hint: `قبل ${original.monthsElapsed} شهر` }
    ]);
  ...
  function renderScenarios() {
    ...
    els.scenarioCards.innerHTML = comparisonCards([
      { label: "الخطة قبل", value: money(derived.original.totalPaid), hint: `${derived.original.monthsElapsed} شهر` },
      { label: "الخطة الآن", value: money(derived.current.totalPaid), hint: `${derived.current.monthsElapsed} شهر` },
  ...
  function drawScenarioChart(whatIf) {
    const data = [
      { label: "قبل", value: derived.original.totalPaid },
      { label: "الآن", value: derived.current.totalPaid },
      { label: "تجربة", value: whatIf.totalPaid }
    ];
  ...
  function importJSONBackup(event) {
    ...
    reader.onload = () => {
      try {
        ...
        openConfirm("سيتم استبدال البيانات الآن بالنسخة المستوردة. هل تريد المتابعة؟", () => {
          state = imported;
          saveStateNow();
          hydrateForms();
          applyPreferences();
  ...
  function recommendStrategy() {
    ...
    if (derived.current.startingPrincipal <= 0) return "أدخل أرقام القرض الأساسية حتى تظهر نتيجة واضحة.";
    ...
    return "إذا توفر معك مبلغ إضافي بدون ضغط، سجله كدفعة مبكرة بطريقة تقليل المدة لأنها غالباً توفر فائدة أكثر.";
  }
